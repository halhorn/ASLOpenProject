'''
frame level のデータを扱う Estimator
category 対応
'''
from typing import Dict
import tensorflow as tf
from .data import read_dataset, adjust_length, CLASS_NUM, CATEGORY_NUM
...
def model_fn(
    ...
    logits = model(features['rgb'], features['audio'])
...
def serving_input_fn():
    feature_placeholders = {
        'rgb': tf.placeholder(dtype=tf.float32, shape=[None, None, 1024], name='serving_rgb'),
        'audio': tf.placeholder(dtype=tf.float32, shape=[None, None, 128], name='serving_audio'),
    }
    rgb, rgb_len = adjust_length(feature_placeholders['rgb'])
    rgb.set_shape([None, None, 1024])
    audio, audio_len = adjust_length(feature_placeholders['audio'])
    audio.set_shape([None, None, 128])
    features = {
        'rgb': rgb,
        'rgb_len': rgb_len,
        'audio': audio,
